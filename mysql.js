...
export default class Server {
  ...
  sendDefinitions(definitions) {
    ...
    for (let definition of definitions) {
      ...
      len = payload.writeUInt8(
        definition.columnType != null
          ? definition.columnType
          : consts.MYSQL_TYPE_VAR_STRING,
        len
      );
      len = payload.writeUInt16LE(
        definition.flags != null ? definition.flags : 0,
        len
      );
      len = payload.writeUInt8(
        definition.decimals != null ? definition.decimals : 0,
        len
      );
  ...
  sendEOF({
    warningCount = 0,
    serverStatus = consts.SERVER_STATUS_AUTOCOMMIT,
  } = {}) {
    // Write EOF
  ...
  helloPacketHandler = (packet) => {
    ...
    return Promise.resolve(
      this.onAuthorize({ clientFlags, maxPacketSize, username, database })
    )
      .then((authorized) => {
